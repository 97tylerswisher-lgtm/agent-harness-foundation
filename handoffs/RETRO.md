...
| --- | --- | --- | --- |
| R0-1 | Scripts are tested on the authoring machine only. Verify `install-kiro.ps1 -Check`, `check-redaction.ps1`, `distill-fixture.ps1`, and `check-spec.ps1` at work; log any execution-policy or encoding failure here. | S0 | OPEN |
...
| R0-3 | `skills/agent-orchestration-workflows/SKILL.md` is 15 KB against the 12 KB skill rule in `steering/30-structure.md`. Trim the pattern table's rationale column or move it to `references/`. | S0 | OPEN |
