...
| R0-5 | Deferred from the S0 practice sweep: (a) a JSON status ledger beside `design.md` (models overwrite JSON less; Kiro owns `tasks.md`, so only the ledger would move); (b) `ponytail.md` steering as `inclusion: fileMatch` on code globs instead of always (AGENTS.md already carries the ruleset always-on); (c) Kiro Correctness for property-style criteria. Decide after three real specs. | S0 | OPEN |
