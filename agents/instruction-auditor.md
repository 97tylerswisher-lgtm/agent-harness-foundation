...
description: Blind format, currency, and bloat auditor for one instruction artifact per spawn: a skill file (skills/<name>/SKILL.md), an agent definition (agents/<name>.md), or a steering file. Use for periodic roster audits, after importing or adapting an instruction file from another project, or before trusting an old skill in a new session. It checks the front matter against the Agent Skills standard, tests every cited path and name for existence, flags foreign-project residue and stale mechanics, and gives a bloat verdict. Spawn one per artifact. It returns a capped structured verdict and never edits.
tools: ["read", "shell"]
resources:
...
`steering/` and the current `handoffs/NEXT_AGENT_HANDOFF.md`. Method, structure, and boundary
...
- Skill (`skills/<name>/SKILL.md`): front matter has `name` (equal to the folder name;
...
- Agent (`agents/<name>.md`): front matter has `name` and `description`; `tools` uses only the
...
- Steering (`steering/*.md`): a front matter `inclusion` field; plain rules; under the shared
...
- Foreign-project tells: product names, machine paths, personal names, tool names, or session
  labels that belong to another project. Check the artifact against `banned-terms.txt`; any hit
...
- Roster row: the artifact's row in `skills/README.md` or `agents/README.md` matches its front
  matter description.
