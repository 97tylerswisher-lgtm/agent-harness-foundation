...
| `agent-orchestration-workflows` | A task needs parallel workers, blind verification, or loop-until-done execution, or risks bloating the orchestrator's context; decides delegate vs inline and routes the spawn | `SKILL.md`, `references/` (delegate-or-inline and spawn-router charts, the seven-pattern table, the plan-critique panel) |
